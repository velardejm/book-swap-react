...
swapRouter.post("/submit-request", authenticateToken, async (req, res) => {
  try {
    const { requesterId, requestedBookId, offerredBookId } = req.body;
    ...
    const checkOfferredBookResult = await queryCheckOfferredBook(
      req.user.userId,
      offerredBookId
    );

    if (checkOfferredBookResult > 0) {
      res.status(409).json({ message: "Offerred book is not available for swap." });
      return;
    }
    ...
    const equivalentCheckResult = await pool.query(sqlCheckEquivalentRequest, [
      req.user.userId,
      offerredBookId,
      requestedBookId,
      "pending",
    ...
    const sqlGetBookOwnerId = "SELECT owner_id FROM books WHERE id=$1";
...
swapRouter.post("/respond", authenticateToken, async (req, res) => {
